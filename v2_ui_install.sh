...
uninstall_old_v2ray() {
    ...
    if [[ -f /usr/local/bin/v2ray ]]; then
        ...
        if [[ $? != 0 ]]; then
            ...
            systemctl stop v2ray
            bash <(curl https://raw.githubusercontent.com/pockyyang/v2-ui/main/install-release.sh) --remove
            systemctl daemon-reload
        fi
    fi
}
...
install_v2ray() {
    ...
    bash <(curl https://raw.githubusercontent.com/pockyyang/v2-ui/main/install-release.sh)
    if [[ $? -ne 0 ]]; then
...
install_v2-ui() {
    ...
    wget -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz https://github.com/pockyyang/v2-ui/releases/download/v5.4.5/v2-ui-linux.tar.gz
    ...
    curl -o /usr/bin/v2-ui -Ls https://raw.githubusercontent.com/pockyyang/v2-ui/main/v2-ui.sh
